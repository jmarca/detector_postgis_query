...
exports.geoQuery =
    function geoQuery(bbox,startend,data,req,next){
        ...
        return function(err, client) {
            if(err){
                console.log('connection error, line 272 d.a.d.r.js '+JSON.stringify(err));
                next(err);
                return;
            }
            ...
            if( req.params.area ){
                bigquery= [
                    ...
                    ,"  ,st_length(st_transform(snipgeom,32611)) * ' + globals.meters_to_miles + ' as len"
            ...
            result.on('row',function(row) {
                ...
                //
                // So here, if this row's component is not yet in the
                // output set, then add it as an array of one element.
                // If it is, then push this val onto that array
                //
                if(!features[row.components]){
                    features[row.components]=[val];
                }else{
                    features[row.components].push(val);
                }
                //
                // The above might be improved/streamlined by just
                // copying the times into a list, and keeping just a
                // single copy of the feature data itself.  Only the
                // start and end times should be different for each
                // array result tied to each features[component].  In
                // otherwords, I could have
                //
                //    features['A,B,C']={'feature':val
                //                       , 'times': [{'ts':start time 1,'endts':endtime 1}
                //                                 , {'ts':start time 2,'endts':endtime 2}
                //                                 , ...
                //                                 , {'ts':start time n,'endts':endtime n} ]
                //                      }
                //
                // But that would require more refactoring than I want to do right now.
                //
