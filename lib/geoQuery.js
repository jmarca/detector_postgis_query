...
function geoQuery(req,next){
    var startend = globals.get_time(req)
    var bbox = globals.get_bbox(req)
    if(bbox===undefined){
        // FIXME hardcoded area, areaid
        bbox = geom_utils.get_area(req,'area','areaid')
    }
    var pn = globals.precision(req.params.zoom)
    if(!pn){pn=14;}
        ...
        return function(err, client) {
            ...
            if( req.params.area ){
                bigquery= [
                    ...
                    ,"  ,st_length(st_transform(fullgeom,32611)) * " + globals.meters_to_miles + " as fulllen"
            ...
            //
                  ,"  join tempseg.tdetector ttd ON (vdsg.detector_id=ttd.detector_id and osm_upgrade.canonical_direction(vdsg.direction)=osm_upgrade.canonical_direction(ttd.direction) and vdsg.refnum=ttd.refnum)"
